...
public static class SetsAndMaps
{
    ...
    public static string[] FindPairs(string[] words)
    {
        ...
        foreach (var word in words)
        {
            ...
            if (wordSet.Contains(reversed) && string.Compare(word, reversed) < 0)
            {
                ...
                wordSet.Remove(reversed); // Remove to avoid duplicates
            }
        }
    ...
    public static Dictionary<string, int> SummarizeDegrees(string filename)
    {
        ...
        foreach (var line in File.ReadLines(filename))
        {
            ...
            if (fields.Length < 4) continue; // Skip lines with insufficient data
    ...
    public static bool IsAnagram(string word1, string word2)
    {
        ...
        {
            if (!letterCounts.ContainsKey(c))
                return false; // Letter not found in first word
            letterCounts[c]--;
            if (letterCounts[c] < 0)
